...
NODE* create_database_node(char* database_name) {
	NODE* n = newnode(N_CREATEDATABASE);

	n->u.CREATEDATABASE.databasename = database_name;
	return n;
}
/*
...
NODE* use_database_node(char* database_name) {
	NODE* n = newnode(N_USEDATABASE);
	n->u.USEDATABASE.databasename = database_name;
	return n;
}
...
*/
NODE *condition_node(NODE *lhsRelattr, int op, NODE *rhsRelattrOrValue)
{
